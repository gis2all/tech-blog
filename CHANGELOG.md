...
后台内容管理与阅读体验改进版本：补齐分类管理和关联文章浏览，统一后台排序与分类选择器，并优化桌面布局、文章排版和精选内容规则。本版本不记录期间新增的文章、项目及分类/标签数据。

### 新增

- 后台分类管理：支持新建、重命名和删除分类，并在分类列表查看关联文章
- 分类与标签列表支持点击查看对应文章，保持 taxonomy 浏览方式一致
- 后台文章分类改用 Decap relation 下拉选择器，直接从分类库选择

### 变更

- 后台文章与草稿默认按更新时间降序；专题与项目按配置顺序，标签与分类按名称升序，并支持按使用量排序
- 精选文章改为仅展示显式标记 `featured: true` 的内容，不再用未标记文章自动补位
- 文章头部展示完整标签；编辑器下拉控件按选项内容自适应宽度
- 前台与后台桌面容器最大宽度统一为 `1480px`，文章正文保持约 `900px` 可读行长
- 前台文章与后台编辑预览共用 Markdown 正文样式，统一标题、表格、行内代码、代码块和媒体排版；行内代码适配亮暗主题

### 修复

- 通过稳定滚动条槽位，修复长短页面切换时顶部布局横向闪动
- 升级 `fast-uri`，修复已知高危依赖安全通告

### 文档与工程

- 补充后台分类管理、排序行为和 relation 分类选择器的接管文档
- 后台分类工作流、taxonomy 关联文章和排序测试覆盖进一步完善
- E2E 文章数量改为从当前内容动态推导，减少对固定内容数量的依赖
