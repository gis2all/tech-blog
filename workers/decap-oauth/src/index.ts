...
//   GET /callback -> exchange code, post token back to the Decap opener
//   anything else -> "Hello 👋" (deployment health check)
//
// Security note (kept from upstream): the callback posts the access token to
// window.opener with targetOrigin "*". This is the upstream default so that
// localhost / preview deployments can log in too. If the admin were ever
// restricted to one origin, targetOrigin should be narrowed accordingly.
