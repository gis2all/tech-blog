...
![在这里插入图片描述](/images/posts/jenkins-zeng-jia-mac-zi-jie-dian/image-04.webp)

### 2\. 使用用户名密码作为凭证

但是在实践的过程中发现有时候使用公私钥的方式总是出错, 所以还有另外一种方式, 那就是使用子节点机器的用户名密码作为凭证. 同样新增一个凭证, 类型选择`Username`, Username和Password为子节点机器的登录名和密码

![在这里插入图片描述](/images/posts/jenkins-zeng-jia-mac-zi-jie-dian/image-05.webp)  
类似这样  
![在这里插入图片描述](/images/posts/jenkins-zeng-jia-mac-zi-jie-dian/image-06.webp)

### 3\. 配置节点工具路径

**由于是在子节点上运行， 所以所有的工具都应使用子节点上工具的路径， 如果没有配置，而直接使用主节点上工具就会出错,这点十分重要！！(比如 Git )**

![在这里插入图片描述](/images/posts/jenkins-zeng-jia-mac-zi-jie-dian/image-07.webp)

## 三、新建节点与测试

![在这里插入图片描述](/images/posts/jenkins-zeng-jia-mac-zi-jie-dian/image-08.webp)  
...
-   启动方式 - **选择`Lauch agents via SSH'`, Host填写子节点的ip地址, 凭证选择之前步骤的凭证(如果其中一个不行的话可以改用另外一个)**  
    ![在这里插入图片描述](/images/posts/jenkins-zeng-jia-mac-zi-jie-dian/image-09.webp)  
    ![在这里插入图片描述](/images/posts/jenkins-zeng-jia-mac-zi-jie-dian/image-10.webp)  
    增加节点后, 我们在主节点的机器上新建一个项目, 在项目配置中选择刚刚建好的节点 , 然后进行测试 , 最终完成节点的增加  
    ![在这里插入图片描述](/images/posts/jenkins-zeng-jia-mac-zi-jie-dian/image-11.webp)  
    ![在这里插入图片描述](/images/posts/jenkins-zeng-jia-mac-zi-jie-dian/image-12.webp)
