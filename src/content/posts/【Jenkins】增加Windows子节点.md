...
cover: /images/posts/【Jenkins】增加Windows子节点/cover.webp
coverAlt: 在这里插入图片描述
...
-   Agent protocols 选项，勾选 Inbound TCP Agent Protocol/4 (TLS encryption)  
    ![在这里插入图片描述](/images/posts/【Jenkins】增加Windows子节点/image-01.webp)
...
-   Use WebSocket，**勾选上这个，`Use WebSocket to connect to the Jenkins master rather than the TCP port. See JEP-222 for background.` 使用WebSocket方式连接至主节点，在子节点上使用TCP连接不知道为什么会出错，所以使用WebSocket方式**  
    ![在这里插入图片描述](/images/posts/【Jenkins】增加Windows子节点/image-02.webp)

**由于是在子节点上运行， 所以所有的工具都应使用子节点上工具的路径， 如果没有配置，而直接使用主节点上工具就会出错,这点十分重要！！(比如 Git )**

![在这里插入图片描述](/images/posts/【Jenkins】增加Windows子节点/image-03.webp)
...
![在这里插入图片描述](/images/posts/【Jenkins】增加Windows子节点/image-04.webp)  
...
-   下载jdk，设置将java将入Path环境变量  
    ![在这里插入图片描述](/images/posts/【Jenkins】增加Windows子节点/image-05.webp)

### 2\. 连接主节点

在主节点的配置中，我们已经复制 `slave-agent,jnlp`文件和`agent.jar`文件到子节点的根目录中，如图  
![在这里插入图片描述](/images/posts/【Jenkins】增加Windows子节点/image-06.webp)  
...
![在这里插入图片描述](/images/posts/【Jenkins】增加Windows子节点/image-07.webp)  
![在这里插入图片描述](/images/posts/【Jenkins】增加Windows子节点/image-08.webp)
