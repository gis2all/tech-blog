# 知行技术博客项目上下文

> 本文件是项目的长期接管说明。新会话、Claude/Codex 或其他开发者在开始工作前应先阅读本文，再检查仓库实际状态。实现发生变化后，同步更新本文中的“当前状态”“已确认决策”和“待确认事项”。

## 1. 项目概览

- 项目名：知行
- 定位：个人技术学习博客，同时作为个人作品集和工程实践记录
- 当前仓库：`D:\Code\tech-blog`
- 目标读者：关注编程、AI Agent、Web 工程、DevOps 和开发效率的中文技术读者
- 核心价值：记录真实问题的发现、排查、解决和复盘，而不只展示最终答案
- 品牌语：知行合一 · 持续积累
- 当前作者身份：gis2all，头像位于 `public/images/avatar-gis2all.png`

一句话架构：

> Astro 生成网站，Decap CMS 提供网页写作后台，GitHub 保存代码、文章和图片，部署平台负责构建并发布静态站点。

## 2. 信息来源与优先级

本文由以下来源整理：

1. Codex 会话 `019fa3c9-fce5-7691-a17a-bc014bc72cef` 中关于 Astro、Decap CMS、GitHub 和部署平台的架构讨论。
2. 本机 OpenDesign 项目“知行录 · 现代技术博客首页”。
3. 当前 Git 仓库的实际文件和提交状态。

OpenDesign 原型当前位于：

```text
C:\Users\12620\AppData\Roaming\Open Design\namespaces\release-stable-win\data\projects\627eafd0-47c4-4f7e-9689-39ba21d8bab1
```

该目录是视觉、布局、响应式和交互意图的设计基准，不是生产代码依赖。不要在 Astro 运行时引用这个 AppData 路径，也不要未经评估直接复制原型中的单文件 HTML、内联 CSS 或模拟数据。

两套实现的事实边界：

- OpenDesign 决定视觉层：版式、尺寸、间距、层级、组件外观、桌面/移动布局和明暗主题表现。
- Astro 决定运行层：真实内容、路由、语义 HTML、Content Collections、可访问状态、构建结果和浏览器行为。
- 原型中的数字、文章、进度和链接只有接入真实数据后才能进入 Astro；不能为了截图一致伪造生产数据。
- 视觉调整必须先更新并验证 OpenDesign，再同步 Astro，最后在相同视口和主题下对比；不得长期让两边各自演进。

出现冲突时按以下顺序判断：

1. 项目所有者最新明确要求。
2. 本文件记录的已确认产品和架构决策。
3. 视觉、布局和响应式问题以已确认的 OpenDesign 原型为准。
4. 内容、路由、语义、数据和运行行为以仓库中已验证的 Astro 实现为准。
5. 早期会话中的探索性建议。

## 3. 当前实际状态

截至 2026-07-29，当前生产分支为 `main`：

- `main` 已包含项目上下文、架构设计稿、实施计划和 Astro 第一版生产代码。
- 已有 `package.json`、`astro.config.mjs`、`tsconfig.json`、`netlify.toml` 和 `package-lock.json`。
- 已建立 Astro Content Collections：`posts`、`series`、`projects`。
- 本地当前已有 106 篇 Markdown 文章：105 篇为从 CSDN `DynastyRumble` / `gis2all` 账号迁移来的公开文章，`git-team-workflow.md` 为 `draft: true` 草稿；早期 5 篇测试文章已删除。
- 已实现首页、文章详情、分类、标签、归档、专题、项目、关于、搜索、RSS、站点地图和 404。
- 项目页功能已实现，但 `src/content/projects/*.json` 仍是第一版示例/占位项目数据，正式对外作为作品集前需要替换为真实项目。
- 已实现 Decap CMS 最小后台：`public/admin/index.html` 和 `public/admin/config.yml`。
- 已实现 Pagefind 静态搜索，生产构建后输出到 `dist/pagefind`。
- OpenDesign 原型目录在本机保留为参考，并通过 `.gitignore` 排除，不作为生产运行时代码。
- `docs/` 目录只作为本地规划、草稿和 Agent 交接资料，通过 `.gitignore` 排除，不进入 GitHub 仓库。
- 2026-07-28 已完成首页和文章页第一轮 UI 同步：统一导航、移动站点操作、真实首页统计、精选复盘、文章元信息、相邻文章、作者说明和移动目录。
- 首页会随 Content Collections 展示当前公开文章；2026-07-29 已完成 CSDN 公开列表 105 / 105 篇迁移，本地仅保留 `git-team-workflow.md` 一个草稿。右栏继续只展示真实分类与标签，不展示虚构浏览量、学习进度或归档填充项。
- OpenDesign 继续保留更丰富的演示正文用于排版覆盖；Astro 只渲染仓库中真实 Markdown，内容长度不同不视为 UI 不一致。

后续会话不得根据原型截图宣称功能已完成。必须以仓库代码、构建结果和浏览器验证为准。

最近验证结果（2026-07-29 CSDN 全量迁移与可见性修复后）：

```text
npm test       → 8 test files passed, 168 tests passed
npm run build  → 431 pages built, dist/posts has 105 public article pages
Pagefind       → indexed 104 pages; one image-only article has almost no searchable text
```

本轮已覆盖完整单元测试和生产构建；尚未重新运行 `npm run test:e2e` 或 `npm run check`。继续改 UI、脚本或迁移工具时，应按风险补跑。

浏览器验证：

- OpenDesign 临时预览地址以本机 OpenDesign 实际输出为准；原型目录仍以第 2 节记录为基准。
- Astro 本地开发固定且仅使用 `http://127.0.0.1:4321/`；不要同时保留多个开发服务，也不要临时切到 4323 等其他端口。若 4321 被占用，先确认并停止/复用该服务，再继续。
- 之前 UI 回归覆盖过首页和文章页；后续迁移内容或调整 UI 时，应以当前保留文章重新跑桌面/移动、浅色/深色浏览器验收。
- 首页只有一个 H1，左右侧栏具名，移动菜单包含 GitHub、主题和后台入口，无横向溢出。
- 文章页标题为桌面 42px、移动 27px；移动标题无孤立单字尾行，分类、发布、更新和阅读时长各出现一次。
- 文章前后篇、作者说明、代码复制和移动目录均可操作；菜单和目录的 ARIA 展开状态、焦点循环/归还以及 `Esc` 关闭行为已验证。
- 移动文章目录入口真实位于 Header DOM 中、处于菜单按钮之前，不靠 fixed 定位伪装顺序，也不覆盖正文；分类、标签和专题页保留文章的完整标签，只有首页显式限制为 2 个标签。
- OpenDesign 与 Astro 首页、文章页已在相同视口对比；结构一致，真实内容差异按职责边界保留。
- 本轮页面严重/致命 Axe 问题为 0，浏览器控制台错误为 0。
- `/search/?q=Agent` 可加载 Pagefind 并返回文章结果。
- `/admin/` GitHub OAuth 登录已验证，可进入 Decap CMS 后台。
- Decap CMS 后台发布闭环已验证：新建测试文章会写入 GitHub commit，并触发 Netlify 自动部署到前台。
- Decap CMS 媒体上传闭环已验证：后台上传图片会写入 `public/images/uploads/`，并可通过 Netlify 静态 URL 访问。
- 2026-07-28 的测试文章和测试图片仅用于验证链路，验证后已从生产内容和线上站点中清理。
- `/posts/git-team-workflow/` 返回 404，证明草稿未生成公开文章页。
- GitHub 远程仓库：`gis2all/tech-blog`。
- Netlify 站点 URL：`https://gis2all-blog.netlify.app`。

## 4. 产品原则

### 4.1 整体基调

“知行”的第一版基调已经确定：这是一个长期积累型的个人技术博客，而不是资讯站、营销落地页、作品集橱窗或教程搬运站。

核心气质是：

- 克制、清爽、可信，有一点个人温度。
- 信息密度高，但层次清楚，读者能快速扫读和进入细节。
- 重视真实工程现场：问题、判断、失败、修复、复盘和可迁移经验。
- 视觉上现代但不炫技，不依赖巨大 Hero、渐变光球、玻璃拟态或装饰性动效撑场面。
- 内容上偏“工作台”和“知识库”，而不是“流量内容池”。
- 文案可以有个人判断，但不能夸张、标题党或假装权威。

一句话描述：

> 知行是一个干净、扎实、可长期维护的中文技术记录系统，用来保存真实学习、调试和工程实践。

后续所有页面、组件和内容调整都应服务这个基调。如果某个设计看起来更酷但降低了可读性、可维护性或真实感，应优先回到克制、清晰、工程化的方向。

### 4.2 内容原则

- 使用真实中文标题、摘要和技术示例，不使用 `lorem ipsum`。
- 文章应保留失败过程、判断依据、适用版本和更新记录。
- 重点是可复用的工程经验，而不是新闻聚合或营销内容。
- 未接入真实数据前，不展示伪造的浏览量、阅读量或增长指标。
- 草稿不得进入生产构建。
- 允许使用第一人称，但要服务于复盘和判断，不写成情绪化流水账。
- 文章标题优先明确问题和场景，不追求猎奇标题。

### 4.3 体验原则

- 参考现代、干净、个人化的 CSDN 信息结构，但删除广告、弹窗、登录诱导、冗余悬浮按钮和杂乱推荐。
- 首页信息密度高但层次清楚，以标题、摘要和元信息为主体。
- 这是工作型技术博客，不是营销落地页：不要巨大 Hero、装饰性大卡片或无意义视觉效果。
- 桌面优先，同时完整支持移动端和键盘操作。
- 深色模式、搜索、筛选、目录和后台编辑不是静态装饰，必须具备可用交互。
- Decap CMS 默认后台样式第一版可以接受；第一版优先保证写作、上传、提交和部署闭环，不把“自定义漂亮后台”作为主线目标。

### 4.4 内容所有权

- Markdown、图片和代码都应保存在项目自己的 GitHub 仓库中。
- CMS 只是编辑和提交界面，不是内容的最终数据库。
- 即使未来替换 Astro、Decap CMS 或部署平台，内容仍应可直接读取和迁移。

## 5. 目标技术架构

### 5.1 职责边界

| 组件 | 职责 | 不负责 |
| --- | --- | --- |
| Astro | 页面生成、Markdown 渲染、内容路由、SEO、RSS、站点地图和前端交互 | 登录、数据库、CMS 权限 |
| Decap CMS | `/admin` 写作后台、字段表单、Markdown 编辑、图片上传和 Git 提交 | 长期保存文章、站点托管 |
| GitHub | 保存源代码、Markdown、图片和完整版本历史 | 渲染网页、运行 CMS 界面 |
| 部署平台 | 拉取仓库、执行 Astro 构建、托管 `dist/`、HTTPS、CDN 和预览部署 | 内容管理、Git 历史、评论和浏览量 |

### 5.2 发布数据流

```text
作者在 /admin 写文章和上传图片
                ↓
Decap CMS 通过 GitHub API 提交文件
                ↓
GitHub 保存 Markdown、图片和 Commit 历史
                ↓
部署平台检测 main 分支的新提交
                ↓
执行依赖安装和 npm run build
                ↓
Astro 读取内容集合并输出 dist/
                ↓
静态资源通过 HTTPS 和 CDN 上线
```

### 5.3 部署平台是可替换层

第一版默认部署平台确认使用 Netlify。Cloudflare Pages 不是硬依赖，部署层仍保持可替换。候选方案：

- Cloudflare Pages：全球 CDN 和 Cloudflare 生态方便；Decap 的 GitHub OAuth 通常需要单独处理。
- Netlify：第一版默认选择；Astro 静态部署顺畅，Decap 相关资料最多，但不把 deprecated 的 Git Gateway 作为架构硬依赖。
- Vercel：Astro 和预览部署体验良好，但 Decap GitHub OAuth 仍需独立配置。
- GitHub Pages：可通过 GitHub Actions 构建，平台数量少，但预览和 OAuth 能力较弱。
- 对象存储 + CDN 或自建 Nginx：控制力高，但运维责任更大，不作为第一版优先方案。

在部署平台正式确定前，业务代码不得绑定某个平台专有 API。静态输出默认以 `dist/` 为交付边界。

### 5.4 第一版已确认架构决策

第一版采用“静态优先、Git 驱动、模块化单体”的架构：

- 不引入数据库，Markdown、图片、配置和代码都以 GitHub 仓库为唯一事实来源。
- 先按单作者博客设计，Decap CMS 发布后直接提交到 `main`。
- 部署平台第一版使用 Netlify，部署边界保持为 `npm run build` 和 `dist/`。
- 文章草稿通过 frontmatter 的 `draft: true` 控制，生产构建默认排除草稿。
- 部署层只接收 GitHub Commit，执行 `npm run build`，发布 Astro 输出的 `dist/`。
- 搜索优先采用 Pagefind 这类静态索引方案，随构建产物一起发布。
- 评论、浏览量、用户登录、邮件订阅、定时发布和离线同步不进入第一版核心架构。

这些决策可以降低第一版复杂度。未来如果要支持多人审核、数据库动态能力或更复杂 CMS 权限，应先更新本文，再改代码。

### 5.5 代码分层与依赖方向

生产实现建议分为五层：

| 层 | 推荐位置 | 负责内容 |
| --- | --- | --- |
| 内容层 | `src/content/posts`、`src/content/series`、`src/content/projects` | Markdown、frontmatter、项目数据和专题数据 |
| 领域层 | `src/lib/content/*` | 草稿过滤、排序、分页、标签/分类统计、相关文章、专题顺序 |
| 展示层 | `src/pages`、`src/layouts`、`src/components`、`src/styles` | 路由页面、布局、组件、主题、响应式和交互 |
| 写作层 | `public/admin/index.html`、`public/admin/config.yml` | Decap CMS 后台入口、字段配置、媒体目录、GitHub 后端配置 |
| 部署层 | 部署平台配置、环境变量、构建命令 | 拉取仓库、安装依赖、执行构建、发布 `dist/`、生成预览地址 |

依赖方向应保持单向：

```text
src/pages
  ↓
layouts / components
  ↓
src/lib/content
  ↓
Astro Content Collections
  ↓
Markdown / images / config
```

原则上，组件只接收已经整理好的展示数据，不直接扫描内容目录；内容查询、草稿过滤、排序和统计集中放在 `src/lib/content/*`，避免首页、分类页、标签页、归档页各写一套规则。

## 6. 首要技术风险：CMS 认证闭环

Decap CMS 能否修改 GitHub 是第一优先级验证项。保护 `/admin` 页面和获得 GitHub 写权限是两个不同问题。

- Cloudflare Access 只能限制谁能打开 `/admin`，不会自动赋予 GitHub API 权限。
- 若使用 Cloudflare Pages，通常需要独立 GitHub OAuth 服务，可部署为 Cloudflare Worker。
- 若 OAuth 配置成本不可接受，可评估 Netlify 的相关能力或 Keystatic 等 Git 驱动 CMS。

正式投入完整页面开发前，至少完成一次最小闭环：

```text
打开 /admin
  → 完成 GitHub 登录
  → 新建测试文章
  → 上传测试图片
  → GitHub 出现 Commit
  → 部署平台自动构建
  → 生产或预览地址出现新文章
```

如果此闭环未通过，不能把“网页后台可发布”标记为完成。

## 7. 信息架构与页面地图

OpenDesign 原型包含以下页面。Astro 实现时应转换为真实路由、布局和组件，而不是继续维护独立的重复 HTML。

### 7.1 前台页面

| 页面 | 建议路由 | 主要职责 |
| --- | --- | --- |
| 首页 | `/` | 作者信息、分类、文章流、热门内容、标签、学习进度和归档入口 |
| 文章详情 | `/posts/[slug]/` | Markdown 正文、目录、系列导航、代码块、更新记录和前后篇 |
| 搜索 | `/search/` | 关键词搜索、排序、结果列表和空状态 |
| 分类 | `/categories/`、`/categories/[slug]/` | 分类统计、分类筛选和文章列表 |
| 标签 | `/tags/`、`/tags/[slug]/` | 标签信息、相关文章和全部标签入口 |
| 归档 | `/archive/` | 按年份和月份组织文章 |
| 专题 | `/series/`、`/series/[slug]/` | 连续学习路径、章节状态和进度 |
| 项目 | `/projects/` | 项目状态、技术栈、演示、GitHub 和复盘入口 |
| 关于 | `/about/` | 作者介绍、当前关注方向和写作原则 |
| 404 | `/404.html` | 返回首页和搜索入口 |

### 7.2 后台与规范页面

| 页面 | 建议路由 | 主要职责 |
| --- | --- | --- |
| 文章管理 | `/admin/` | 文章统计、标题搜索、状态筛选、编辑和预览 |
| 文章编辑器 | CMS 内部路由 | Markdown 写作、实时预览、元数据、图片和发布 |
| 图片资源 | CMS 内部路由 | 搜索、目录筛选、上传、压缩和链接复用 |
| 发布状态 | CMS 或项目工具页 | 保存、上传、部署、网络和内容异常状态 |
| 设计系统 | 开发文档或 Storybook 类页面 | Token、排版、按钮、状态、代码、图片和边界用例 |
| 原型地图 | 仅设计参考 | 汇总所有原型页面，不必作为生产公开页面 |

## 8. 核心页面规格

### 8.1 全局导航

- 固定顶部导航，高度约 62-64px；移动端约 56px。
- 品牌 Logo“知行”。
- 主导航：首页、分类、归档、专题、项目、关于。
- 宽搜索框，支持回车进入搜索页。
- GitHub、深色模式和进入后台入口。
- 移动端收纳主导航，并提供独立搜索展开按钮。

### 8.2 首页

桌面最大内容宽度约 1280px，采用三栏布局：

- 左栏约 220px：头像、作者、简介、品牌语、文章/分类/标签统计和技术分类。
- 中栏自适应，约 700px：最新、热门、专栏筛选，紧凑文章列表和分页。
- 右栏约 280px：精选复盘、标签云和当前建设说明。

文章列表应包含：

- 标题和两行以内摘要。
- 标签、发布日期、阅读时长。
- 可选的 120×80 缩略图；无封面时文字区域自然扩展。
- 条目之间使用分割线，不把每篇文章做成浮动大卡片。
- 轻微 hover 状态。

首页交互：

- 搜索输入跳转完整搜索页；原型可保留实时过滤用于验证列表状态。
- 无匹配结果时显示清晰空状态。
- 深色模式持久化。
- 移动端隐藏左右栏，保留文章主流程。
- 所有统计、精选文章和标签必须由公开内容计算，不展示虚构的阅读量、趋势或进度。

### 8.3 文章详情

桌面采用左侧专题导航、中间正文、右侧目录的三栏结构：

- 面包屑、文章类型、分类、标题、摘要、作者、发布日期、更新时间和阅读时长。
- 可选的“实践验证”“难度”等真实徽标，不可伪造。
- 左侧只显示真实存在的专题说明和章节顺序；没有真实数据时不补全虚构章节、进度或仓库链接。
- 右侧目录随滚动高亮，显示阅读进度并提供返回顶部。
- 移动端隐藏侧栏，目录改为顶部导航图标入口和底部抽屉；入口不得覆盖正文。

正文需要支持：

- H2/H3 层级和锚点目录。
- 行内代码、带语言和复制按钮的代码块、行高亮和横向滚动。
- 信息、结论、警告、错误等提示块。
- 引用、列表、表格和长配置折叠。
- 图片、图注、多图和灯箱，支持 `Esc` 关闭。
- Mermaid、公式和在线 Demo；Demo 失败时显示外链。
- 参考资料、仓库引用和文章修改历史。
- 上一篇、下一篇和作者信息。

### 8.4 文章管理后台

- 左侧后台导航：文章管理、新建文章、图片资源、发布状态、设计系统和查看博客。
- 统计：全部文章、草稿、已发布；阅读数据只有接入真实统计后才展示。
- 文章表格：标题、状态、分类、更新时间和操作。
- 支持标题搜索以及已发布、草稿、定时发布状态筛选。
- 移动端后台导航使用可关闭抽屉；宽表格明确支持横向滚动。

### 8.5 文章编辑器

- 桌面端 Markdown 编辑与实时预览各占一栏。
- 工具栏至少支持粗体、H2、链接、图片和代码块。
- 显示自动保存状态、字数、预计阅读时间、目标分支和 GitHub 连接状态。
- 文章设置包括摘要、分类、标签、封面、Slug 和封面替代文本。
- 图片可上传或从图片库选择。
- 发布前显示目标分支和预计流程，并要求显式确认。
- 远端版本更新时不得静默覆盖，需合并或创建副本。
- 移动端使用“编辑/预览”分段切换，并提供稳定的底部发布按钮。

### 8.6 图片资源

- 支持文件名搜索和目录筛选。
- 展示文件名、尺寸和体积。
- 上传入口支持拖拽、选择文件、压缩中、成功和失败状态。
- 默认转为 WebP，生成安全且唯一的英文文件名。
- 上传成功后可复制公开路径或插入文章。

## 9. 内容模型

建议使用 Astro Content Collections，并以构建时 Schema 校验 frontmatter。第一版文章字段：

```yaml
---
title: 用 Astro 搭建个人博客
description: 记录 Astro 内容集合和部署过程
publishedAt: 2026-07-27
updatedAt: 2026-07-27
category: 前端工程
tags:
  - Astro
  - 博客
cover: /images/posts/astro-blog/cover.webp
coverAlt: Astro 内容与发布流程示意图
draft: false
series: 个人博客工程
seriesOrder: 1
---
```

约束：

- `title`、`description`、`publishedAt`、`category`、`tags` 和 `draft` 必填。
- URL Slug 默认由文件名决定，也可在 Schema 中显式提供，但必须唯一且稳定。
- `updatedAt`、`cover`、`coverAlt`、`series`、`seriesOrder` 可选；有封面时 `coverAlt` 必填。
- 生产构建排除 `draft: true`。
- 日期统一使用 `YYYY-MM-DD`，构建失败时给出具体文件和字段。
- 分类是单值主分类，标签是多值；专题是有顺序的连续内容。

建议目录：

```text
src/content/posts/
├─ jenkins-groovy-practices.md
├─ pc-dian-nao-ruan-jian-bian-cheng-huan-jing-cha-jian-tui-jian.md
├─ ...
└─ git-team-workflow.md  （draft）

public/images/posts/
├─ jenkins-groovy-practices/
├─ pc-dian-nao-ruan-jian-bian-cheng-huan-jing-cha-jian-tui-jian/
└─ ...
```

## 10. 图片规则

- 每篇文章使用独立图片目录。
- 文件名使用英文、小写和短横线。
- 默认转为 WebP；保留透明度等需求时可按实际格式处理。
- 封面建议 1280×720 或统一 16:9。
- 正文图片宽度通常不超过 1600px。
- 单张压缩后尽量控制在 500KB 以内；后台硬限制原型为 5MB。
- 原始超大图片不进入 Git 仓库。
- 所有有信息含义的图片必须有替代文本。

## 11. 视觉系统

最终品牌优化版优先于早期原型中的纯科技蓝：

视觉方向应围绕“技术笔记、知识沉淀、工程复盘”建立，而不是 SaaS 官网或炫技作品集。页面应像一个可靠的个人工作台：边界清楚、留白克制、文字优先、状态明确。

| Token | 值 | 用途 |
| --- | --- | --- |
| Brand Ink | `#18324A` | Logo、后台侧栏、深色封面基底 |
| Brand Accent | `#0B7285` | 主操作、链接、进度和当前状态 |
| Accent Strong | `#075E6E` | 主按钮 hover |
| Brand Soft | `#E7F5F7` | 标签、选中态和轻提示背景 |
| Brand Warm | `#F4B860` | 少量警告提示，不作为主色 |
| Canvas | `#F5F6F7` | 浅色页面背景 |
| Panel | `#FFFFFF` | 内容面板 |
| Text | `#20242B` | 主要正文 |
| Muted | `#68717B` | 辅助信息 |
| Border | `#E3E7EB` | 分割线和边框 |
| Success | `#16845B` | 成功状态 |
| Warning | `#BD6718` | 警告状态 |
| Error | `#C63D48` | 错误和危险操作 |

设计约束：

- 中文无衬线字体，使用系统字体栈。
- 文章大标题约 34px，章节标题约 25px，列表标题约 17px，辅助信息约 11-12px。
- 边框细、阴影轻、圆角 4-8px，默认约 6px。
- 不使用大圆角、玻璃拟态、紫蓝渐变、装饰性光球或巨大 Hero。
- 不嵌套无意义卡片；页面分区保持紧凑和可扫描。
- 封面可使用墨蓝到蓝绿色基底和克制网格纹理，但必须承载真实主题信息。
- 暗色模式需要覆盖正文、代码、提示块、表格、后台和弹窗。
- 所有交互元素必须有清晰的 `:focus-visible` 状态。

## 12. 响应式与可访问性

- 主要断点参考：1280px 内容上限、约 1080/1120px 隐藏次要右栏、约 780/900px 进入移动布局、约 520px 调整紧凑排版。
- 移动端不能只缩小桌面布局；应收纳导航、隐藏次要栏并保留核心流程。
- 长标题、长标签、长代码、表格和大量内容必须保持可读，不得撑破布局。
- 搜索、弹窗、抽屉和移动目录支持键盘与 `Esc`。
- 模态框使用 `role="dialog"`、`aria-modal="true"`，打开后移动焦点并限制焦点循环，关闭后归还焦点。
- 视觉顺序必须与 DOM 和键盘焦点顺序一致；移动控件在进入桌面断点并隐藏前，应把焦点转移到可见的桌面等价入口。
- 上传区域应可聚焦并提供按钮语义。
- 尊重语义化 HTML，文章列表使用 `<article>`，导航使用 `<nav>`，时间使用 `<time>`。
- 验证桌面和移动端截图，检查非空白、无重叠、无横向溢出和可操作状态。

## 13. 状态与错误处理

必须为以下流程设计明确状态，而不是只有成功路径：

- 自动保存：保存中、已保存、保存失败、本地草稿安全、恢复网络后重试。
- 图片上传：压缩进度、上传成功、超过限制、格式错误和网络失败。
- 发布部署：提交 GitHub、构建中、发布成功、构建失败和可查看日志。
- 内容校验：Frontmatter 字段错误时显示文件、字段、行号和期望格式。
- 并发编辑：检测远端更新，要求合并或创建副本。
- 内容状态：空草稿、无搜索结果、文章不存在和 404。
- 离线状态：允许继续本地编辑，联网后同步；实际能力需按 CMS 支持程度实现，不得只展示文案。

部署平台名称不能硬编码在通用状态组件中。原型中的“Cloudflare Pages 正在构建”应改为由部署配置提供平台名称。

## 14. SEO、订阅与构建要求

第一版应包含：

- 每页唯一的 `<title>` 和 description。
- canonical URL、Open Graph 和 Twitter Card。
- 文章页结构化数据。
- `sitemap.xml`、`robots.txt` 和 RSS。
- 代码高亮和稳定的标题锚点。
- 草稿排除和内容 Schema 构建校验。
- 自定义 404。
- 静态输出到 `dist/`。

评论、登录用户系统和浏览量统计不属于第一版基础能力。需要时采用外部服务或独立方案，并先确认隐私、成本和数据所有权。

## 15. 推荐实施顺序

1. 初始化 Astro 和 TypeScript，建立内容集合 Schema、基础布局、设计 Token、Lint/格式化和构建脚本。
2. 用少量真实 Markdown 样例打通首页、文章详情、分类、标签、归档、专题、项目、关于和 404。
3. 实现搜索、深色模式、目录、代码复制、图片灯箱和响应式导航。
4. 建立 Decap CMS 最小配置，并优先完成 GitHub OAuth、文章提交、图片提交和自动部署闭环。
5. 完成后台字段、媒体规则、错误状态、冲突保护和无障碍细节。
6. 添加 RSS、站点地图、SEO、部署预览和生产域名配置。
7. 使用浏览器端到端验证桌面与移动端关键路径，然后再扩展评论、统计或高级 CMS 能力。

当前项目已完成第一版最小闭环：Astro 前台、Decap 后台、GitHub 保存、Netlify 构建发布、文章发布测试、图片上传测试和 105 篇 CSDN 文章全量迁移均已验证。后续优先进入前台细节、正式域名、内容整理、线上发布检查，以及评论、统计或高级 CMS 能力评估。

第一阶段验收不是“页面看起来像原型”，而是：

```text
真实 Markdown → Astro 构建 → 页面可访问
后台创建文章和图片 → GitHub Commit → 自动部署 → 新文章上线
```

## 16. 开发与验证约定

- Windows 环境默认使用 PowerShell 语义，不使用 Bash `&&`。
- Astro 本地开发端口固定为 `4321`，标准地址为 `http://127.0.0.1:4321/`。后续会话不要为了测试或预览另起 4323 等其他端口；需要重启时先停止旧的 4321 服务再启动。
- 编辑 Markdown、JSON、YAML、HTML、TS/TSX 时优先使用 `apply_patch`，保持 BOM-free UTF-8。
- 不覆盖用户未提交的修改，不进行无关重构。
- 页面实现优先复用 Astro 布局和组件，不复制多份导航、文章行、侧栏或状态样式。
- 内容数据必须来自 Content Collections，不把原型中的 48 篇、12.8k 阅读等示例数值当作生产数据。
- OpenDesign 是视觉、布局、响应式和交互意图的设计基准；Astro 是内容、路由、语义和运行行为的事实来源。
- 涉及 UI 的工作固定执行：先改 OpenDesign → 验证桌面/移动和浅色/深色 → 同步 Astro → 用相同视口截图对比 → 检查交互、溢出、控制台和可访问性。
- 若 OpenDesign MCP 可用，优先通过 MCP 定位和修改项目；MCP 不可用时可直接编辑本文记录的本机项目目录，但仍需在 OpenDesign 或浏览器中重新验证。
- 不能只改 Astro 后把原型留在旧状态，也不能把原型的模拟数据复制进生产代码来追求像素一致。
- 每次涉及页面或交互的变更至少运行构建；自动化测试按风险、用户要求或关键流程补跑，不要求每次都完整执行。当前 UI 回归命令为 `npm run test:e2e`。
- 完成前使用真实浏览器检查桌面和移动端，重点验证导航、搜索、筛选、文章目录、后台编辑、弹窗和发布流程。
- 若新增或替换 CMS、部署平台、评论、统计或搜索服务，更新本文的架构、环境变量和迁移说明。

## 17. 待确认事项

以下事项尚未由代码或正式配置锁定，开始相关工作前应向项目所有者确认，或在最小验证后记录决策：

- 正式独立域名和社交链接。
- 是否需要评论、浏览量或隐私友好的分析工具。
- 原型中的定时发布和离线同步是否纳入第一版；这两项不能只靠静态 Astro 页面完成。
- 图片转换是在浏览器、CMS 插件、CI 还是独立图片服务中执行。

已确认的第一版约束：

- 无数据库。
- 单作者。
- 作者身份已确认为 `gis2all`，头像为 `public/images/avatar-gis2all.png`，GitHub 仓库和站点入口为 `https://github.com/gis2all/tech-blog`。
- 第一版部署平台使用 Netlify。
- 第一版 CMS 使用 Decap CMS 的 `github` backend。
- Decap CMS 直接提交 `main`。
- GitHub OAuth 登录已通过 Netlify Authentication Provider 验证；第一版不额外实现自定义 OAuth Worker。
- 静态部署，第一版使用 Netlify 负责构建并发布 `dist/`。
- 生产构建通过 `draft: true` 排除草稿。
- `docs/`、OpenDesign 原型和其他本地规划资料不进入 GitHub 仓库。

## 18. 新会话接管清单

后续会话开始时按以下顺序快速恢复上下文：

1. 阅读本文件。
2. 运行 `git status --short`，保留所有现有修改。
3. 查看 `package.json`、`astro.config.*`、`src/content.config.*` 和当前目录结构；若文件不存在，以“当前实际状态”为准。
4. 检查最近提交和已有测试，不重复已经完成的工作。
5. 对照“待确认事项”判断当前任务是否依赖未决选择。
6. 实现后运行与风险相称的测试和构建，并更新本文件中已经过时的状态。

不要仅凭本文件声称实现存在；本文件解释项目全貌，仓库和验证结果证明完成程度。

## 19. CSDN 迁移状态

- 来源账号是 `DynastyRumble` / `gis2all`。
- 2026-07-29 重新确认真实全量来源不是账号页首屏 20 篇，而是 CSDN 接口 `community/home-api/v1/get-business-list` 返回的 `total: 105`。
- 当前本地已覆盖 105 / 105 篇：其中 20 篇为前序迁移已存在/跳过，85 篇由本轮批量迁移成功补齐；剩余未迁移文章为 0。
- 全量迁移后曾有 10 篇补迁文章因单篇迁移脚本默认 `draft: true` 而未进入首页统计，表现为首页只显示 95 篇；2026-07-29 已统一恢复为 `draft: false`，当前公开文章数为 105。
- 后半段 43 篇不是文章不存在，而是 Node/普通浏览器新会话抓取详情页时被 CSDN WAF 拦截。最终使用已登录的 `csdn-pilot` Playwright 会话逐篇缓存详情页 HTML，再用 `.migration/csdn/migrate-all-remaining.ts` 从缓存生成 Markdown 和本地图片。`.migration/` 已被 `.gitignore` 排除，不要提交 CSDN 登录态、原始 HTML 缓存或临时批量脚本。
- 已修正迁移工具的全量迁移问题：详情页 `upDate` 作为 `updatedAt` 来源；短正文允许直接生成短摘要；栏目为空时按关键词兜底分类；CSDN 长 GIF 录屏保留原文件并允许最多 300 帧；profile 页抓取失败不再阻塞详情页已有更新时间的文章；图片型文章允许以标题兜底摘要；误标为 GIF 的 PNG 按真实解码格式处理；详情正文增加截断检测，避免再次把被 WAF 或页面异常截断的正文当作完整文章写入。
- 每篇迁移文落到 `src/content/posts/<slug>.md` 与 `public/images/posts/<slug>/`，再检查首页、分类、标签、归档和搜索。
- 迁移时优先保持原文标题、结构、发布时间、更新时间、图片和链接，不擅自改写正文。
- 迁移脚本仍默认输出草稿；批量脚本会把成功迁移的 CSDN 文章统一改为 `draft: false` 公开，`git-team-workflow.md` 仍是测试草稿。
- 早期 5 篇测试文章已删除，不要再把它们当作正式内容恢复。
